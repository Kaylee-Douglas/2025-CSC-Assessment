...
def startgame():
    ...
    msgbox(f"Nice to meet you, {username}.")
...
while running:
    ...
    def travel():
        ...
        direction = enterbox("What direction would you like to travel? ").strip().title()
        ...
        if direction in direction_map:
            ...
            if next_area_number > 0:  # If there is something in this direction
                ...
                msgbox("Hm, there doesn't appear to be anything this way. Maybe try another direction?")
                travel()
        else:
            msgbox("Enter an actual direction - North, South, East, or West.") # if this occurs user entered an invalid input.
            travel()
    ...
    if area_1["active"]: #Because value is a boolean, it isn't required to specify what you're checking for
        if objective == "blank":
            ...
            if acceptquest == "Yes":
                ...
                confirm = enterbox("Are you sure? This option will end the game. (Yes/No)").strip().title()
                while confirm not in ["Yes", "No"]:
                    msgbox("Please enter either yes or no")
                    confirm = enterbox("Are you sure you don't want to help? (Yes/No)")
                if confirm == "Yes":
                    quit()
            objective = "find koro"
            msgbox("Objective gained: find your Koro.")
            msgbox("Travel south to leave your home and begin your journey.")
            msgbox("You walk outside, ready to begin your journey.")
            travel()
        else:
    ...
    if area_2["active"]:
        ...
        msgbox(f"You find yourself at a {area_2['title']}, which could be more accuratly described as {area_2['description']}.")
        msgbox("There doesn't appear to be anything to do here, but the road extends to the east.")
        travel() 
    ...
    if area_3["active"]: 
        ...
        msgbox(f"Your travels brings you to {area_3['description']}. This {area_3['title']} could be useful!")
        inspect = enterbox('Inspect? (Yes/No)').strip().title()
    ...
    if area_4["active"]: 
        ...
        msgbox(f'''Before you lies {area_4["description"]}. This {area_4['title']} is non descript - but in the distance to the south, you can clearly make out the shapes of a town.''')
        travel()

    # area five

    if area_5["active"]:
        msgbox(f"You find yourself in front of a {area_5['description']}.")
        if objective == "find koro":
            msgbox("This doesn't seem like Koro's neighbourhood - you can't spot his house. You should keep looking.")
            travel()
        if objective == "find shop":
            msgbox("Huh. No shops here. You should keep looking. The same pair of friends are chatting by the gate.")
            talk = enterbox("Do you want to talk to them again? (Yes/No)").strip().title()
            while talk not in ["Yes", "No"]:
                msgbox("Please enter either 'Yes' or 'No'.")
                talk = enterbox("Do you want to talk to them again? (Yes/No)")
            if talk == "Yes":
    ...
    if area_6["active"]:
        ...
        msgbox(f"Finally, you've made your way into a more populated area. The {area_6['title']}, {area_6['description']}")
        if inspect == "Yes":
            msgbox("This must be the market mentioned by the signpost.")
    ...
    # area seven
    if area_7["active"]:
        msgbox(f"This {area_7['description']} must be the {area_7['title']}. You can buy the things Koro needs here.")
    ...
    if area_8["active"]:
        ...
        msgbox(f"In between two modern neighourhoods, a {area_8['description']} lies. {area_8['title']} seems to stretch endlessly into the south and west.")
        travel()
    ...
    if area_9["active"]:
        if objective == 'find koro':
            ...
            while ask not in ["Yes", "No"]:
                msgbox("Please enter either 'Yes' or 'No'.")
                ask = enterbox("Ask Koro what he needs from the shop? (Yes/No)").strip().title()
            if ask == "Yes":
            ...
            msgbox(f'Forgot what I asked for already? It was {random_item}.')
            msgbox("He shuts the door - huh. Guess he didn't feel like talking.")
            travel()
        elif objective == 'find shop':
            msgbox("Oh, you found the name? Now head to the shop and bring it back!")
            msgbox("He shuts the door - huh. Guess he didn't feel like talking.")
            travel()
